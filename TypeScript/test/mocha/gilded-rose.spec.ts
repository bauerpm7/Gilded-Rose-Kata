    ...
    qualitiesToTest.forEach(quality => {
      sellInValuesToTest.forEach(sellIn => {
        it(`updates generic item quality correctly with name ${ITEM_TYPES.GENERIC}, quality: ${quality}, sellin: ${sellIn}`, () => {
          ...
          if (sellIn > 0) {
            if(quality >= 1) expect(items[0].quality).to.equal(quality - 1);
            if(quality <= 0) expect(items[0].quality).to.equal(0);
          };
          if (sellIn <= 0) {
            if(quality >= 2) expect(items[0].quality).to.equal(quality - 2);
            if(quality < 2) expect(items[0].quality).to.equal(0);
          };
        });
      });
    });
    ...
    qualitiesToTest.forEach(quality => {
      sellInValuesToTest.forEach(sellIn => {
        it(`updates Brie quality correctly with name ${ITEM_TYPES.BRIE}, quality: ${quality}, sellin: ${sellIn}`, () => {
          ...
          expect(items[0].sellIn).to.equal(sellIn - 1);
          if (sellIn > 0) {
            if(quality < 50) expect(items[0].quality).to.equal(quality + 1);
            if(quality >= 50) expect(items[0].quality).to.equal(50);
          };
          if (sellIn <= 0) {
            if(quality < 48) expect(items[0].quality).to.equal(quality + 2);
            if(quality >= 48) expect(items[0].quality).to.equal(50);
          };
        });
      });
    });
    ...
    qualitiesToTest.forEach(quality => {
      sellInValuesToTest.forEach(sellIn => {
        it(`updates generic item quality correctly with name ${ITEM_TYPES.PASSES}, quality: ${quality}, sellin: ${sellIn}`, () => {
          ...
          const items = gildedRose.updateQuality();
          expect(items[0].sellIn).to.equal(sellIn - 1);
          if (sellIn <= 0) expect(items[0].quality).to.equal(0)
          if (sellIn > 10) {
            if (quality < 50) {
    ...
    qualitiesToTest.forEach(quality => {
      sellInValuesToTest.forEach(sellIn => {
        it(`updates generic item quality correctly with name ${ITEM_TYPES.CONJURED}, quality: ${quality}, sellin: ${sellIn}`, () => {
          ...
          if (sellIn <= 0) {
            if(quality >= 4) expect(items[0].quality).to.equal(quality - 4);
            if(quality < 4) expect(items[0].quality).to.equal(0);
          };
        });
      });
    });
